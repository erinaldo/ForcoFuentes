...
Public Class frmRepConsumoAct
    ...
    Private Function funCargaCombos()
        ...
        '-- Dependencia
        strSQL = clsCombos.funGetLkActividadAll()
        funCargarlue(Me.lkAct, strSQL)
        '-- Year
    ...
    Private Sub bPrint_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles bPrint.ItemClick
        If Validar() = True Then
            ...
            lstParametros.Add(Me.lkAct.EditValue.ToString)
            lstParametros.Add(strUser.ToString)
            '--
            If Me.optOpcion.SelectedIndex = 0 Then
                If Me.opt2.SelectedIndex = 0 Then
                    funPrintConSp("repConsumoActCTAB.rpt", lstParametros)
                Else
                    funPrintConSp("repConsumoActCTAB3.rpt", lstParametros)
                End If
            Else
                If Me.opt2.SelectedIndex = 0 Then
                    funPrintConSp("repConsumoActCTAB2.rpt", lstParametros)
                Else
                    funPrintConSp("repConsumoActCTAB4.rpt", lstParametros)
                End If
            End If
