...
Public Class frmRepVentas_Open
    ...
    Private Function funUpdateGrid() As Boolean
        ...
        Dim vnExpress As Integer = 0

        If Me.opt.SelectedIndex = 0 Then
            strSQL = "SELECT * FROM vw_GB_Ventas " &
                        "WHERE CAST(dtmFechaDoc AS DATE) Between '" & strRango1 & "' " &
                        " AND '" & strRango2 & "' " &
                        " AND nEmpresa = " & intEmpresa &
                        " ORDER BY nNumero"
        ElseIf Me.opt.SelectedIndex = 1 Then
            vnExpress = 0
            strSQL = "SELECT * FROM vw_GB_Ventas " &
                "WHERE CAST(dtmFechaDoc AS DATE) Between '" & strRango1 & "' " &
                " AND '" & strRango2 & "' " &
        ...
        indice = 0
        '--
        funSetColumna(GridView1, "nNumero", "Factura", funIndice(), 60, 1)
        ...
        funSetColumna(GridView1, "Costo_For_Venta", "P.Costo", funIndice(), 100, 1, , , "n2", 3, , DevExpress.Utils.FormatType.Numeric)
        funSetColumna(GridView1, "Costo_Total", "Costo_Total", funIndice(), 100, 1, , , "n2", 3, , DevExpress.Utils.FormatType.Numeric)
        Return True
    End Function
