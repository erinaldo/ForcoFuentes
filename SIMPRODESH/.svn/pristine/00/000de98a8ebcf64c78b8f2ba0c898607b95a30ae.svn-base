...
Public Class frmConsultaPorDepartamento
    Dim m As Integer
    Private Sub frmConsultaPorDepartamento_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        consultadepto()
    End Sub
    Private Sub consultadepto()
        Dim sqlcomando As New OleDb.OleDbCommand
        Dim sqlstring As String
        Dim i As Integer
        Try
            abrirconexion()
            sqlstring = "SELECT ca.Depto_Nombre+' - '+convert(VARCHAR,ca.Depto_Id) departamento FROM dbo.Departamento ca (NOLOCK) where ca.Emp_Id=" + com(empresa) + "order by ca.Depto_Nombre"
            ...
            For i = 0 To dt.Rows.Count - 1
                ListBox1.Items.Add(dt.Rows(i)("departamento"))
            Next
            cerrarconexion()
    ...
    Private Sub BackgroundWorker1_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker1.DoWork
        ...
        Try
            ...
            Dim sqlstring As String
            abrirconexion()
            sqlstring = "select a.Articulo_Id, a.Articulo_Nombre from dbo.Articulo a , dbo.Departamento c where a.Depto_Id=c.Depto_Id and c.Depto_Nombre+' - '+convert(VARCHAR,c.Depto_Id) in" & filtros + " order by a.Articulo_Nombre"
            sqlcomando.CommandText = sqlstring
            sqlcomando.CommandType = CommandType.Text
            sqlcomando.Connection = Conexion
            Dim dt As New DataTable
