Public Class frmLogin
    ...
    End Sub
    Private Sub funParametros()
        '-- Datos de la transacción
        strSQL = "SELECT * FROM GB_Parametros"
        '--
        Dim dsParametros As DataSet = funFillDataSet(strSQL)
        '--
        If dsParametros.Tables(0).Rows.Count > 0 Then
            With dsParametros.Tables(0).Rows(0)
                '-- Permite mover el pedido de Pendiente a Preparacion
                bolMovePedido = .Item("SiNo")
            End With
        End If
        '--
    End Sub
