...
Public Class frmDespachos_Re
    Dim ds As New DataSet
    Dim vn_Pedido As Integer
    Dim vn_Sucursal As Integer
    Dim vn_SubPedido As Integer

    Private Sub frmDespachos_Re_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        funInivar()
    End Sub
    ...
    Private Function funCargaSucursal()
        ...
        If Me.lkFormato.EditValue = 1 Then
            strSQL = "SELECT Suc_Id AS nCodigo, Suc_Nombre AS strDescripcion FROM Sucursal WHERE Suc_Nombre LIKE '%MM%' ORDER BY strDescripcion"
        ElseIf Me.lkFormato.EditValue = 2 Then
            strSQL = "SELECT Suc_Id AS nCodigo, Suc_Nombre AS strDescripcion FROM Sucursal WHERE Suc_Nombre LIKE '%MC%' ORDER BY strDescripcion"
        ElseIf Me.lkFormato.EditValue = 3 Then
            strSQL = "SELECT Suc_Id AS nCodigo, Suc_Nombre AS strDescripcion FROM Sucursal WHERE Suc_Nombre LIKE '%SHOPPERS%' ORDER BY strDescripcion"
        End If
        '--
    ...
    Private Sub funGetPedido()
        With Me.gvDetalle
            If .FocusedRowHandle >= 0 Then
                ...
                vn_SubPedido = funNull2Val(.GetFocusedRowCellValue("SubPedido"))
            Else
    ...
    Private Function funPrint()
        ...
        '--
        dsReporte = clsPedidos.funDespacho_BarCodeRe(vn_Sucursal, vn_Pedido, vn_SubPedido)
        '--
        ...
        Next
        '-- Reporte
        '-- funPrintXml(dsReporte, "Pedido_Ticket_v3.rpt")
    ...
    Private Function funUpdateGrid() As Boolean
        ...
        '--
        Me.gcDetalle.DataSource = Nothing
        ds = clsPedidos.funRepDespachosListaRe(CInt(lkSucursal.EditValue))
        '-- 
        ...
        funSetColumna(gvDetalle, "SubPedido", "SubPedido", funIndice(), 90, 1)
