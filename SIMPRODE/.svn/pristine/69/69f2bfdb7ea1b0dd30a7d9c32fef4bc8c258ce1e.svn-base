...
Public Class frmRepVentasDetalleCat
    '--
    ...
    Private Sub frmRepVentasDetalleCat_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyData = Keys.Enter Then
            SendKeys.Send("{TAB}")
        End If
    End Sub

    Private Sub frmRepVentasDetalleCat_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        funUpdateGrid_Depto()
        funUpdateGrid_Suc()
    End Sub
    ...
    Private Function funUpdateGrid_Depto()
        Try
            ds = clsCategoria.funListaCategoria_All()
    ...
    Private Function funUpdateGrid_Suc()
        Try
            ds = clsSucursal.funListaSucursalSinCEDI()
        ...
        With Me.gvLista
            For i As Integer = 0 To .RowCount - 1
                If Convert.ToBoolean(.GetDataRow(i)("optSelection")) = True Then
                    ...
                    strChekados += .GetDataRow(i)("Categoria_Id").ToString
                    nCambios += 1
                End If
            Next
    ...
    Private Sub funUpdateGrid_Datos()
        ...
        Try
            ...
            indice = 0
            '--
            funSetColumna(gvDetalle2, "SUC_ID", "SUC_ID", funIndice(), 90, 1)
            funSetColumna(gvDetalle2, "SUC_NOMBRE", "SUC_NOMBRE", funIndice(), 90, 1)
            funSetColumna(gvDetalle2, "CAT_ID", "CAT_ID", funIndice(), 90, 1)
            funSetColumna(gvDetalle2, "CATEGORIA", "CATEGORIA", funIndice(), 90, 1)
            funSetColumna(gvDetalle2, "VENTA_UNIDAD", "VENTA_UNIDAD", funIndice(), 100, 1, , , "n0", 0, , DevExpress.Utils.FormatType.Numeric)
            funSetColumna(gvDetalle2, "COSTO_TOTAL", "COSTO_TOTAL", funIndice(), 100, 1, , , "n2", 3, , DevExpress.Utils.FormatType.Numeric)
            funSetColumna(gvDetalle2, "VALOR_VENTAS", "VALOR_VENTAS", funIndice(), 100, 1, , , "n2", 3, , DevExpress.Utils.FormatType.Numeric)
            funSetColumna(gvDetalle2, "MARGEN_UTILIDAD", "MARGEN_UTILIDAD", funIndice(), 100, 1, , , "n2", 3, , DevExpress.Utils.FormatType.Numeric)
            funSetColumna(gvDetalle2, "% MARGEN UTILIDAD", "% MARGEN UTILIDAD", funIndice(), 100, 1, , , "P", 3, , DevExpress.Utils.FormatType.Numeric)
            funSetColumna(gvDetalle2, "EXISTENCIA", "EXISTENCIA", funIndice(), 100, 1, , , "n0", 0, , DevExpress.Utils.FormatType.Numeric)
    ...
    Private Sub funCargaData2()
        ...
        Dim CMD As New SqlCommand("sp_RepVentasDetalle2_Porras_Consol_Cat")
        '--
    ...
    Private Sub gcDetalle2_Click(sender As Object, e As EventArgs) Handles gcDetalle2.Click
