...
Partial Class frmOC_Detalle
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            'Metodo
            CType(Me.Master, MasterCRM3).PageTitle = ":: Solicitud Pago de Suministros - Autorizar ::"
    ...
    Private Sub funCargaData()
        ...
        If ds.Tables("Tabla").Rows.Count >= 1 Then
            ...
            '-- Si es 1. Pendiente, 3. Rechazado, 4. Autorizado, 5 Pagado
            If Me.lkEstado.SelectedValue = 1 Or lkEstado.SelectedValue = 3 Or lkEstado.SelectedValue = 4 Or lkEstado.SelectedValue = 5 Then
        ...
        '--
    End Sub
    Protected Sub funUpdate_Imagenes()
        '-- ASPxImageSlider1.ImageSourceFolder = "~\Content\Images\photo_gallery\"
        ASPxImageZoomNavigator1.ImageSourceFolder = "~/Content/Images/photo_gallery"
    ...
    Protected Sub funUpdate_Autoriza()
        ...
        clsForcoHelper.AbrirConexionGlobal()
        Session("nOrden") = Request.Params("nOrden").ToString
        Dim vnPago_Stauts As Integer = 4
        '--
        strSQL = String.Format("UPDATE Orden_Local_Encabezado SET nPago_Status={0}, dtmFecha_SolPagoAuto='{1}', " &
                                    "nUsuario_SolPagoAuto={2}, strUsuario_SolPagoAuto='{3}' WHERE Emp_ID ={4} AND Suc_Id ={5} AND Orden_Id={6}",
    ...
    Protected Sub funUpdate_Rechaza()
        ...
        clsForcoHelper.AbrirConexionGlobal()
        Session("nOrden") = Request.Params("nOrden").ToString
        Dim vnPago_Stauts As Integer = 3
        '--
        strSQL = String.Format("UPDATE Orden_Local_Encabezado SET nPago_Status={0}, dtmFecha_SolPagoNo='{1}', " &
                                    "nUsuario_SolPagoNo={2}, strUsuario_SolPagoNo='{3}' WHERE Emp_ID ={4} AND Suc_Id ={5} AND Orden_Id={6}",
                                    vnPago_Stauts, clsForcoHelper.funFechaSql(varToday, True), vnUsuario, Page.User.Identity.Name, 1, 1001, Session("nOrden"))
        '--
        clsForcoHelper.funRunSQLTransaccion(strSQL)
        '--
